...
A random number between 0 and 100 is generated and stored in `x`.  **Pass** will be printed if `x` is >50. **Check** will be printed if `x` is >30 and <= 50. **Fail** will be printed otherwise.
