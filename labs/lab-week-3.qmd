...
<br>
Today's tutorial has two purposes. Firstly, it aims to acquaint you with an API for data retrieval, enhance your expertise in using the `tidyverse` for data cleansing, investigate diverse output formatting techniques, and finalize the session with a simple scheduling procedure to automate API requests. Secondly, it focuses on familiarizing you with web scraping.

For the API, we will use the [MoviesDatabase](https://rapidapi.com/SAdrian/api/moviesdatabase/details) from [RapidAPI](https://rapidapi.com/hub). You will need to create an account on RapidAPI to access the APIs. If you prefer using another API, feel free to change.
